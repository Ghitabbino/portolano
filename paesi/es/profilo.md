...
<div style="font-weight:800;color:var(--accent)">¡Bienvenido a bordo!</div>
<div id="profilo-dati" style="margin:10px 0;line-height:1.6">Caricamento…</div>
<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
<button onclick="modificaProfil()" style="padding:7px 12px;border-radius:8px;background:var(--accent);color:#06231f;font-weight:800;border:1px solid var(--accent);cursor:pointer">Editar datos</button>
<button onclick="logout()" style="padding:7px 12px;border-radius:8px;background:transparent;color:var(--accent);font-weight:700;border:1px solid var(--line);cursor:pointer">Salir</button>
<button onclick="disiscrivi()" style="padding:7px 12px;border-radius:8px;background:#3a1a1a;color:#ffb9b9;font-weight:700;border:1px solid #d32f2f;cursor:pointer">Baja — borrar todo</button>
</div>
</div>

<div style="border:1px solid var(--line);border-radius:10px;padding:14px;background:#0b131b;margin:14px 0">
<div style="font-weight:700">Tus contribuciones</div>
<div style="margin:8px 0">
- **Insignia actual:** <span id="badge-liv">Novato ⚓</span><br>
- **Fichas actualizadas:** <span id="badge-count">0</span><br>
- **Historial:** tus modificaciones se trazan a tu nombre para moderación
</div>
</div>

<div id="alert-section" style="border:2px solid var(--accent);border-radius:12px;padding:16px;background:#0b131b;margin:14px 0">
<div style="font-weight:800;color:var(--accent);font-size:16px;margin-bottom:6px">🔔 Tus áreas de alerta</div>
<div style="font-size:12px;color:var(--muted);margin-bottom:10px">Marca la <b>bandera</b> para recibir alertas de todo el nivel. Haz clic en el <b>nombre</b> para abrir las zonas inferiores.</div>
...
<div style="margin:12px 0 6px;font-weight:700;color:var(--ink)">Canales de recepción</div>
...
<div style="margin:12px 0 6px;font-weight:700;color:var(--ink)">Frecuencia</div>
<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:12px">
<label style="display:flex;gap:6px;align-items:center;padding:7px 10px;border:1px solid var(--line);border-radius:8px;background:#16222e;cursor:pointer"><input type="checkbox" id="ch-istantaneo" value="istantaneo" checked> Instantáneo — solo críticos L3 🚨</label>
<label style="display:flex;gap:6px;align-items:center;padding:7px 10px;border:1px solid var(--line);border-radius:8px;background:#16222e;cursor:pointer"><input type="checkbox" id="ch-settimanale" value="settimanale"> Resumen semanal</label>
</div>
<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
<button onclick="salvaAlert()" style="padding:10px 18px;border-radius:8px;background:var(--accent);color:#06231f;font-weight:800;border:1px solid var(--accent);cursor:pointer">💾 Guardar selección</button>
...
<a id="btn-contribuisci" href="#contribuisci" onclick="goContribuir(event)" style="display:inline-block;padding:14px 28px;border-radius:12px;background:linear-gradient(135deg,#4db6ac,#2e7d6f);color:#fff;font-weight:900;text-decoration:none;box-shadow:0 4px 12px rgba(0,0,0,.3)">✏️ Contribuir al derrotero</a>
</div>
...
function renderProfil(){
  ...
  box.innerHTML=`<b>Nickname:</b> ${saved.nick||'—'}<br><b>Email:</b> ${saved.email}<br><b>Barca:</b> ${saved.barca||'—'}<br><b>Aree sel.:</b> ${aree}<br><b>Paesi:</b> ${paesi}<br><b>Canali:</b> ${chans}<br><b>Frecuencia:</b> ${(saved.freq||[]).join(', ')||'—'}`;
...
function disiscrivi(){if(confirm('¿Borrar todos los datos locales?')){localStorage.removeItem('sailtropics_user');localStorage.removeItem('sailtropics_logged');renderProfil(); renderAlertTree(); alert('Datos borrados (demo local). En producción borrado definitivo en servidor y RGPD.');}}
...
function renderAlertTree(){
  ...
  const logged=localStorage.getItem('sailtropics_logged')==='1';
  if(!logged){ treeEl.innerHTML='<div style="color:var(--muted);font-size:13px;padding:8px">Accede para seleccionar las áreas.</div>'; return; }
...
function updateAlertCount(){
  ...
  const el=document.getElementById('alert-count'); if(el) el.textContent=c?`${c} seleccionadas`:'ninguna selección';
}
function salvaAlert(){
  ...
  if(chans.length===0){ const m=document.getElementById('alert-msg'); m.style.display='block'; m.style.background='#3a1a1a'; m.style.border='1px solid #d32f2f'; m.style.color='#ffb9b9'; m.textContent='Selecciona al menos un canal (Mail, WhatsApp o Telegram).'; return; }
  if(freq.length===0){ const m=document.getElementById('alert-msg'); m.style.display='block'; m.style.background='#3a1a1a'; m.style.border='1px solid #d32f2f'; m.style.color='#ffb9b9'; m.textContent='Selecciona al menos una frecuencia.'; return; }
  const saved=JSON.parse(localStorage.getItem('sailtropics_user')||'{}');
  ...
  m.innerHTML=`✅ Guardado: ${sel.length} áreas, canales: ${chans.join(', ')}, freq: ${freq.join(', ')}`;
  renderProfil();
...
> **Privacidad — nuestro compromiso:** tus datos se usan solo para las alertas seleccionadas y nunca se cederán a terceros.

[ Volver a Áreas](00-indice.md) · [Offline & GPX](offline-gpx.md)
