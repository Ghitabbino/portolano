# 02 — Cost of Living

**Last updated: 27/08/2026**

Currency: **euro (EUR)**. Livello prezzi: **DATA MISSING** — to be verified con fonti locali aggiornate.

> ⚠️ Le voci di questo elenco sono le stesse per tutti i paesi del portolano. Dove il dato non è stato ancora trovato dalle fonti si scrive **DATA MISSING**: viene ricercato al controllo mensile (vedi [00 Indice](../00-indice.md)).

## Groceries & Galley Provisioning

| Item | Indicative Price | Notes |
|---|---|---|
| Birra locale supermercato (0,33 l) | **DATA MISSING** | **DATA MISSING** |
| Caffè al bancone | **DATA MISSING** | **DATA MISSING** |
| Baguette / pane | **DATA MISSING** | **DATA MISSING** |
| Acqua imbottigliata 1,5 l | **DATA MISSING** | **DATA MISSING** |
| Latte 1 l | **DATA MISSING** | **DATA MISSING** |
| Riso 1 kg | **DATA MISSING** | **DATA MISSING** |
| Pollo fresco 1 kg | **DATA MISSING** | **DATA MISSING** |

## Eating Out (Average)

| Item | Indicative Price |
|---|---|
| Menu ristorante medio | **DATA MISSING** |
| Birra al bar | **DATA MISSING** |
| Menu fascia alta | **DATA MISSING** |
| Snack / tapas da strada | **DATA MISSING** |

Prezzi specifici per locale → [10 Restaurants](10-ristoranti.md).

## Fuel

| Item | Indicative Price | Notes |
|---|---|---|
| Gasolio | **DATA MISSING** | **DATA MISSING** |
| Benzina SP95 | **DATA MISSING** | **DATA MISSING** |
| Benzina SP98 | **DATA MISSING** | **DATA MISSING** |
| Bombola gas (scambio) | **DATA MISSING** | **DATA MISSING** |

## Transport & Connections

| Item | Indicative Price | Notes |
|---|---|---|
| Bus locale | **DATA MISSING** | **DATA MISSING** |
| Collegamento marittimo locale | **DATA MISSING** | **DATA MISSING** |
| Noleggio auto giornaliero | **DATA MISSING** | **DATA MISSING** |
| Taxi | **DATA MISSING** | **DATA MISSING** |

## Everyday Services

| Item | Indicative Price | Notes |
|---|---|---|
| Lavanderia self-service | **DATA MISSING** | **DATA MISSING** |
| SIM prepagata con dati | **DATA MISSING** | **DATA MISSING** |
| Lavaggio scafo (hull dive) | **DATA MISSING** | **DATA MISSING** |

## Cash & Payments

Euro, carte accettate diffusamente; **DATA MISSING** su dettagli locali (commissioni, ATM).

## Further Reading

- Tariffe complete marine e boe → [03 Ports & Marinas](03-porti-ancoraggi.md)
- Ristoranti con prezzi specifici per locale → [10 Restaurants](10-ristoranti.md)

Last updated: 27/08/2026
