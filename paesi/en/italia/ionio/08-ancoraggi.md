# 08 — Anchorage Pilot

**Last checked: 27/08/2026** — Ionio

## General Anchoring Rules (First and Foremost)

- Àncora libera consentita fuori dalle zone portuali regolamentate, dai campi boe balneari e dalle **Aree Marine Protette (AMP)** — dove ancoraggio spesso vietato o consentito solo su boe. Verificare ordinanze locali Capitaneria per Ionio ★★★★★
- Divieto di ancoraggio su **posidonia oceanica** (fanerogama protetta) — sanzioni elevate. Verificare sempre natura del fondale ★★★★★
- Rispettare i corridoi di lancio / zone balneazione (ordinanza balneare comunale) ★★★★
- Segnaletica **IALA-A** in Italia (opposta ai Caribbean) ★★★★★

### ⛔ Zone di divieto / AMP in Ionio

| Area | Prohibition | Source |
|---|---|---|
| **DATA MISSING** — AMP / riserva in Ionio | **DATA MISSING** — verificare ordinanza | **DATA MISSING** |
| Zone balneazione / campi boe | Ancoraggio vietato nei corridoi segnalati | Ordinanza balneare comunale ★★★★ |

Testi e mappe ufficiali: ordinanze Capitaneria di Porto competente per Ionio e sito Ministero Ambiente per AMP.

## Summary Table — ancoraggi di Ionio

| # | Anchorage | Shelter | Holding | Depth | Crowding | Verdict |
|---|---|---|---|---|---|---|
| 1 | **DATA MISSING** | **DATA MISSING** | **DATA MISSING** | **DATA MISSING** | **DATA MISSING** | **DATA MISSING** |

*Ancoraggi specifici di Ionio da verificare su Portolano IIM, Navionics e fonti locali — tabella da popolare con dati verificati.*

### General Anchorage Map

<div class="mapframe" data-slug="ionio" data-minz="6" data-maxz="16" data-lat="38.50" data-lon="17.00" data-markers='[]'></div>

*Mappa centrata su Ionio (38.50N 17.00E, indicativo zona). Marker ancoraggi verificati con coordinate WGS84: **DATA MISSING** — tasselli locali zoom 6–13 (satellitare / carta nautica / segnali OpenSeaMap). Nessuna coordinata inventata: i pin saranno aggiunti solo dopo verifica su fonte attendibile.*

---

## Official Charts

| Chart | Coverage | Notes |
|---|---|---|
| **IIM — Carte nautiche italiane** | Ionio | Istituto Idrografico della Marina — WGS84 |
| **Portolano IIM P-series** | Coste italiane | Descrizione ufficiale approdi e ancoraggi |
| NV Charts / Navionics / C-MAP | Mediterraneo | Cartografia elettronica di supporto |

- Le carte IIM riportano zone regolamentate e AMP; acquistabili presso rivenditori autorizzati.
- Per la navigazione quotidiana: carta elettronica aggiornata sul plotter + carta cartacea IIM come riserva.

## Not Included (For Now)

- Dettaglio ancoraggi minori di Ionio: **DATA MISSING** — da verificare

## Anchor Checklist per Ionio

1. Ordinanza Capitaneria / AMP scaricata e verificata ✓/ **DATA MISSING**
2. Natura fondale verificata (sabbia/fango — mai posidonia) ✓
3. Corridoi balneazione e campi boe evitati ✓
4. Previsione vento/mare per 24h controllata (Meteomar) ✓
5. Carta IIM a bordo ✓

Last updated: 27/08/2026