...
REGIONE_PER_PAESE = {
    ...
    'bonaire': 'Caraibi', 'cayman': 'Caraibi', 'colombia': 'Caraibi', 'cuba': 'Caraibi',
...
MACRO_CARAIBI = {
    ...
    'belize': 'Coste dell’America Centrale', 'colombia': 'Coste dell’America Centrale', 'honduras': 'Coste dell’America Centrale',
...
BANDIERE = {
    ...
    'anguilla': '🇦🇮', 'antigua-barbuda': '🇦🇬', 'aruba': '🇦🇼', 'azzorre': 'assets/flags/azzorre.svg',
    'bahamas': '🇧🇸', 'barbados': '🇧🇧', 'belize': '🇧🇿',     'bonaire': '🇧🇶',
    'cabo-verde': '🇨🇻', 'canarie': '🇮🇨', 'cayman': '🇰🇾', 'colombia': '🇨🇴', 'costarica': '🇨🇷',
    'cuba': '🇨🇺', 'curacao': '🇨🇼', 'dominica': '🇩🇲', 'giamaica': '🇯🇲',
    'grenada': '🇬🇩', 'grenadine': '🇻🇨', 'guadalupa': '🇬🇵', 'honduras': '🇭🇳',
    'haiti': '🇭🇹', 'madeira': 'assets/flags/madeira.svg', 'repubblica-dominicana': '🇩🇴', 'martinica': '🇲🇶', 'montserrat': '🇲🇸',
    'nicaragua': '🇳🇮', 'panama': '🇵🇦', 'porto-rico': '🇵🇷', 'saba': 'assets/flags/saba.svg',
...
def render(md_path: Path, title: str, sec_id: str, country: str = ""):
    ...
    # Nota download solo sulla prima pagina di ogni paese (00-ingresso)
    if country and md_path.name == "00-ingresso-visti.md":
        ckey = country.split("/")[0]
        try:
            offline_ref = OFFLINE_PID
        except NameError:
            offline_ref = "offline"
    ...
    if 'ancoraggi' in title.lower() and country:
        # bottoni GPX + ZIP per paese (file generati da export_gpx.py / export_zip.py)
        ckey = country.split("/")[0]  # per zone Canarie/Capo Verde
        try:
            offline_ref2 = OFFLINE_PID
        except NameError:
            offline_ref2 = "offline"
...
print("DEBUG iscriviti registered:", (ROOT / "iscriviti.md").read_text()[:50])
ISCRIVITI_PID = queue[-1][2]
register(ROOT / "accedi.md", "Accedi", country="", in_nav=False)
ACCEDI_PID = queue[-1][2]
register(ROOT / "profilo.md", "Profilo", country="", in_nav=False)
PROFILO_PID = queue[-1][2]
...
.loc-badge { color:var(--accent); font-weight:800; font-size:26px; display:inline-block; cursor:pointer; text-decoration:none; margin:0 0 10px; }
.loc-badge:hover { border-bottom:2px solid var(--accent); }
...
function renderNav(){
  ...
  if(ch.length){
    ...
    else if(n.t==='c'){const f=TREE.flag[n.k]||'🏝️'; ic=f.endsWith('.svg')?'<img src="'+esc(f)+'" alt="" style="width:26px;height:19px;object-fit:cover;border-radius:3px;vertical-align:middle">':esc(f);}
...
function initMaps(root){
  ...
  root.querySelectorAll('.mapframe:not(.lmap)').forEach(el=>{
    ...
    const minz=+(el.dataset.minz||12), maxz=+(el.dataset.maxz||17);
...
document.addEventListener('click',e=>{const bk=e.target.closest('[data-back]');if(bk){e.preventDefault();go(parentOf(state)||'');}});
...
(function(){
  function buildRegTree(){
    const el=document.getElementById('reg-tree');
    if(!el || typeof TREE==='undefined' || !TREE.regions) return;
    let h='';
    TREE.regions.forEach(reg=>{
      if(reg.ready===false) return;
      const flag=reg.i||'🌊';
      h+=`<div style="margin:6px 0"><div style="display:flex;align-items:center;gap:6px"><input type="checkbox" class="reg-check" data-k="${reg.k}"> <span style="font-size:16px">${flag}</span> <a href="#" onclick="event.preventDefault();const n=this.parentElement.nextElementSibling;n.style.display=n.style.display==='none'?'block':'none'" style="color:#fff;font-weight:700;text-decoration:none;font-size:14px">${reg.l}</a></div>`;
      let ch=`<div style="display:none;margin:4px 0 4px 18px;border-left:2px solid var(--line);padding-left:10px">`;
      let has=false;
      (reg.subs||[]).forEach(sub=>{
        has=true;
        ch+=`<div style="margin:5px 0"><div style="display:flex;align-items:center;gap:6px"><input type="checkbox" class="reg-check" data-k="${sub.k}"> <span>${sub.i||'—'}</span> <a href="#" onclick="event.preventDefault();const n=this.parentElement.nextElementSibling;n.style.display=n.style.display==='none'?'block':'none'" style="color:#fff;text-decoration:none;font-size:13px">${sub.l}</a></div>`;
        let sch=`<div style="display:none;margin:3px 0 3px 18px;border-left:1px solid var(--line);padding-left:8px">`;
        let hs=false;
        (sub.p||[]).forEach(pk=>{
          hs=true;
          const f=TREE.flag[pk]||'🏝️';
          const fh=f.endsWith('.svg')?`<img src="${f}" style="width:20px;height:14px;object-fit:cover;border-radius:2px;vertical-align:middle">`:f;
          const lbl=TREE.lbl[pk]||pk;
          sch+=`<div style="margin:3px 0"><div style="display:flex;align-items:center;gap:6px"><input type="checkbox" class="reg-check" data-k="${pk}"> <span style="font-size:14px">${fh}</span> <a href="#" onclick="event.preventDefault();const n=this.parentElement.nextElementSibling;n.style.display=n.style.display==='none'?'block':'none'" style="color:#fff;text-decoration:none;font-size:13px">${lbl}</a></div>`;
          const zks=Object.keys(TREE.zona||{}).filter(k=>k.startsWith(pk+'/'));
          if(zks.length){
            sch+=`<div style="display:none;margin:2px 0 2px 18px;border-left:1px solid var(--line);padding-left:8px">`;
            zks.forEach(zk=>{const zl=zk.split('/').pop().replace(/-/g,' ');const zi=TREE.zona[zk]||'🐬';sch+=`<label style="display:block;margin:2px 0;font-size:13px"><input type="checkbox" class="reg-check" data-k="${zk}"> ${zi} ${zl}</label>`;});
            sch+=`</div>`;
          }
          sch+=`</div>`;
        });
      ...
      (reg.p||[]).forEach(pk=>{
        ...
        const fh=f.endsWith('.svg')?`<img src="${f}" style="width:20px;height:14px;object-fit:cover;border-radius:2px;vertical-align:middle">`:f;
        const lbl=TREE.lbl[pk]||pk;
        ch+=`<div style="margin:3px 0"><div style="display:flex;align-items:center;gap:6px"><input type="checkbox" class="reg-check" data-k="${pk}"> <span style="font-size:14px">${fh}</span> <a href="#" onclick="event.preventDefault();const n=this.parentElement.nextElementSibling;n.style.display=n.style.display==='none'?'block':'none'" style="color:#fff;text-decoration:none;font-size:13px">${lbl}</a></div>`;
        const zks=Object.keys(TREE.zona||{}).filter(k=>k.startsWith(pk+'/'));
        if(zks.length){
          ch+=`<div style="display:none;margin:2px 0 2px 18px;border-left:1px solid var(--line);padding-left:8px">`;
          zks.forEach(zk=>{const zl=zk.split('/').pop().replace(/-/g,' ');const zi=TREE.zona[zk]||'🐬';ch+=`<label style="display:block;margin:2px 0;font-size:13px"><input type="checkbox" class="reg-check" data-k="${zk}"> ${zi} ${zl}</label>`;});
          ch+=`</div>`;
        }
        ch+=`</div>`;
      });
    ...
    el.addEventListener('change',()=>{
      const sel=[...document.querySelectorAll('.reg-check:checked')].map(c=>c.dataset.k);
      const cnt=document.getElementById('reg-sel-count'); if(cnt) cnt.textContent=sel.length;
      const lst=document.getElementById('reg-sel-list'); if(lst) lst.textContent=sel.slice(0,6).join(', ')+(sel.length>6?' +'+(sel.length-6):'');
    });
  }
...
<script>window.onerror=function(m,u,l){try{document.title='ERRORE: '+m+' @'+l;}catch(x){}};</script>
</body>
</html>
"""
...
html_template = (TEMPLATE.replace("__NAV__", nav_html)
        ...
        .replace(f'<section id="{ISCRIVITI_PID}" class="page"',
                 f'<section id="{ISCRIVITI_PID}" class="page" data-special="1"', 1)
        .replace(f'<section id="{ACCEDI_PID}" class="page"',
                 f'<section id="{ACCEDI_PID}" class="page" data-special="1"', 1)
        .replace(f'<section id="{PROFILO_PID}" class="page"',
                 f'<section id="{PROFILO_PID}" class="page" data-special="1"', 1)
        .replace(f'<section id="{OFFLINE_PID}" class="page"',
                 f'<section id="{OFFLINE_PID}" class="page" data-special="1"', 1)
        .replace(f'<section id="{CHI_PID}" class="page"',
                 f'<section id="{CHI_PID}" class="page" data-special="1"', 1)
        .replace(f'<section id="{CONTRIBUISCI_PID}" class="page"',
                 f'<section id="{CONTRIBUISCI_PID}" class="page" data-special="1"', 1)
...
try:
    ...
    # === Merch POD catalogo etico — 3 categorie ===
    merch_md = (ROOT / "merch.md").read_text(encoding="utf-8") if (ROOT / "merch.md").exists() else ""
    if merch_md:
        merch_body = md.convert(merch_md)
        md.reset()
        merch_html = f"""<!DOCTYPE html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Catalogo Merch POD — Portolano × Sailtropics</title><link rel="stylesheet" href="assets/leaflet.css"><style>:root{{--bg:#0f1720;--panel:#16222e;--ink:#dbe7f1;--muted:#8aa2b5;--accent:#4db6ac;--line:#24384a}}*{{box-sizing:border-box}}body{{margin:0;min-height:100vh;background:var(--bg);color:var(--ink);font:15px/1.6 -apple-system,Segoe UI,Roboto,sans-serif;padding:24px;max-width:960px;margin:auto}}a{{color:var(--accent)}}h1{{color:#fff}}h2{{color:#fff;border-bottom:1px solid var(--line);padding-bottom:6px}}h3{{color:var(--accent)}}blockquote{{border-left:3px solid var(--accent);margin:12px 0;padding:10px 14px;background:#0b131b;border-radius:8px;color:var(--ink)}}table{{width:100%;border-collapse:collapse;margin:14px 0;font-size:13.5px;display:block;overflow-x:auto;max-width:100%}}th,td{{border:1px solid var(--line);padding:8px 10px;text-align:left;vertical-align:top}}th{{background:#16222e;color:#fff;white-space:nowrap}}tr:nth-child(even) td{{background:#131e29}}code{{background:#0b131b;border:1px solid var(--line);padding:1px 5px;border-radius:5px;font-size:13px;color:#ffd54f}}</style></head><body><p><a href="paesi.html" style="color:var(--accent);text-decoration:none">← Torna al portolano</a> · <a href="../it/index.html" style="color:var(--accent)">IT</a> · <a href="../en/index.html" style="color:var(--accent)">EN</a> · <a href="trasparenza.html" style="color:var(--accent)">Trasparenza</a> · <a href="merch.json" style="color:var(--muted);font-size:12px">JSON tecnico</a></p>{merch_body}<p style="margin-top:18px;padding:12px;border:1px solid var(--line);border-radius:8px;background:#0b131b;font-size:13px;color:var(--muted)">POD zero stock · Cotone bio GOTS · Inchiostri base acqua · Spedizione EU 3–6gg · <a href="trasparenza.html" style="color:var(--accent)">Trasparenza costi</a> · <a href="ringraziamenti.html" style="color:var(--accent)">Ringraziamenti</a></p></body></html>"""
        (ROOT / "merch.html").write_text(merch_html, encoding="utf-8")
        print(f"OK -> merch.html (catalogo POD 3 categorie)")
except Exception as e:
    print(f"trasparenza/ringraziamenti/merch export skip: {{e}}")
# === GPX + ZIP export ===
try:
    import subprocess, sys
    subprocess.run([sys.executable, str(Path(__file__).parent / "export_gpx.py")], check=False)
    subprocess.run([sys.executable, str(Path(__file__).parent / "export_zip.py")], check=False)
except Exception as e:
    print(f"GPX/ZIP export skip: {e}")
