...
<div style="font-weight:800;color:var(--accent)">Willkommen an Bord!</div>
<div id="profilo-dati" style="margin:10px 0;line-height:1.6">Caricamento…</div>
<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
<button onclick="modificaProfil()" style="padding:7px 12px;border-radius:8px;background:var(--accent);color:#06231f;font-weight:800;border:1px solid var(--accent);cursor:pointer">Daten bearbeiten</button>
<button onclick="logout()" style="padding:7px 12px;border-radius:8px;background:transparent;color:var(--accent);font-weight:700;border:1px solid var(--line);cursor:pointer">Abmelden</button>
<button onclick="disiscrivi()" style="padding:7px 12px;border-radius:8px;background:#3a1a1a;color:#ffb9b9;font-weight:700;border:1px solid #d32f2f;cursor:pointer">Abmelden — alles löschen</button>
</div>
</div>

<div style="border:1px solid var(--line);border-radius:10px;padding:14px;background:#0b131b;margin:14px 0">
<div style="font-weight:700">Deine Beiträge</div>
<div style="margin:8px 0">
- **Aktuelles Abzeichen:** <span id="badge-liv">Novize ⚓</span><br>
- **Aktualisierte Blätter:** <span id="badge-count">0</span><br>
- **Verlauf:** deine Änderungen werden in deinem Namen nachverfolgt zur Moderation
</div>
</div>

<div id="alert-section" style="border:2px solid var(--accent);border-radius:12px;padding:16px;background:#0b131b;margin:14px 0">
<div style="font-weight:800;color:var(--accent);font-size:16px;margin-bottom:6px">🔔 Deine Alarm-Gebiete</div>
<div style="font-size:12px;color:var(--muted);margin-bottom:10px">Kreuze die <b>Flagge</b> an, um Alerts für die ganze Ebene zu erhalten. Klicke auf den <b>Namen</b>, um die Unterzonen zu öffnen.</div>
...
<div style="margin:12px 0 6px;font-weight:700;color:var(--ink)">Empfangskanäle</div>
...
<div style="margin:12px 0 6px;font-weight:700;color:var(--ink)">Frequenz</div>
<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:12px">
<label style="display:flex;gap:6px;align-items:center;padding:7px 10px;border:1px solid var(--line);border-radius:8px;background:#16222e;cursor:pointer"><input type="checkbox" id="ch-istantaneo" value="istantaneo" checked> Sofort — nur kritisch L3 🚨</label>
<label style="display:flex;gap:6px;align-items:center;padding:7px 10px;border:1px solid var(--line);border-radius:8px;background:#16222e;cursor:pointer"><input type="checkbox" id="ch-settimanale" value="settimanale"> Wöchentliche Zusammenfassung</label>
</div>
<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
<button onclick="salvaAlert()" style="padding:10px 18px;border-radius:8px;background:var(--accent);color:#06231f;font-weight:800;border:1px solid var(--accent);cursor:pointer">💾 Auswahl speichern</button>
...
<a id="btn-contribuisci" href="#contribuisci" onclick="goBeitrag(event)" style="display:inline-block;padding:14px 28px;border-radius:12px;background:linear-gradient(135deg,#4db6ac,#2e7d6f);color:#fff;font-weight:900;text-decoration:none;box-shadow:0 4px 12px rgba(0,0,0,.3)">✏️ Zum Handbuch beitragen</a>
</div>
...
function renderProfil(){
  ...
  box.innerHTML=`<b>Nickname:</b> ${saved.nick||'—'}<br><b>Email:</b> ${saved.email}<br><b>Barca:</b> ${saved.barca||'—'}<br><b>Aree sel.:</b> ${aree}<br><b>Paesi:</b> ${paesi}<br><b>Canali:</b> ${chans}<br><b>Frequenz:</b> ${(saved.freq||[]).join(', ')||'—'}`;
  ...
  const bl=document.getElementById('badge-liv'); if(bl) bl.textContent = n>=20?'Master Skipper 🧭★': n>=5?'Navigatore d’Alizé 🧭':'Novize ⚓';
  // init channels
...
function disiscrivi(){if(confirm('Alle lokalen Daten löschen?')){localStorage.removeItem('sailtropics_user');localStorage.removeItem('sailtropics_logged');renderProfil(); renderAlertTree(); alert('Daten gelöscht (lokale Demo). In Produktion endgültige Löschung serverseitig und DSGVO.');}}
function goBeitrag(e){ if(e) e.preventDefault(); const pid=(typeof CONTRIBUISCI_PID!=='undefined'?CONTRIBUISCI_PID:null); if(pid) location.hash='#'+pid; else location.hash='#contribuisci'; }
...
function renderAlertTree(){
  ...
  const logged=localStorage.getItem('sailtropics_logged')==='1';
  if(!logged){ treeEl.innerHTML='<div style="color:var(--muted);font-size:13px;padding:8px">Melde dich an, um Gebiete zu wählen.</div>'; return; }
...
function updateAlertCount(){
  ...
  const el=document.getElementById('alert-count'); if(el) el.textContent=c?`${c} ausgewählt`:'keine Auswahl';
}
function salvaAlert(){
  ...
  if(chans.length===0){ const m=document.getElementById('alert-msg'); m.style.display='block'; m.style.background='#3a1a1a'; m.style.border='1px solid #d32f2f'; m.style.color='#ffb9b9'; m.textContent='Wähle mindestens einen Kanal (Mail, WhatsApp oder Telegram).'; return; }
  if(freq.length===0){ const m=document.getElementById('alert-msg'); m.style.display='block'; m.style.background='#3a1a1a'; m.style.border='1px solid #d32f2f'; m.style.color='#ffb9b9'; m.textContent='Wähle mindestens eine Frequenz.'; return; }
  const saved=JSON.parse(localStorage.getItem('sailtropics_user')||'{}');
  ...
  m.innerHTML=`✅ Gespeichert: ${sel.length} Gebiete, Kanäle: ${chans.join(', ')}, Freq: ${freq.join(', ')}`;
  renderProfil();
...
> **Datenschutz — unser Versprechen:** deine Daten werden nur für die gewählten Alerts verwendet und niemals an Dritte weitergegeben.

[ Zurück zu Gebiete](00-indice.md) · [Offline & GPX](offline-gpx.md)
