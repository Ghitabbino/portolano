...
function initMaps(root){
  ...
  root.querySelectorAll('.mapframe:not(.lmap)').forEach(el=>{
    ...
    if(!hasPts&&!hasZones){
      ...
      const fl=span>0.6?0:(span>0.25?9:12);
...
<script>window.onerror=function(m,u,l){try{document.title="ERRORE: "+m+" @riga "+l;var d=document.createElement("div");d.style.cssText="position:fixed;inset:auto 0 0 0;z-index:99999;background:#b71c1c;color:#fff;padding:10px;font:13px monospace";d.textContent=document.title;(document.body||document.documentElement).appendChild(d);}catch(e){}};</script>
</body>
</html>
"""
